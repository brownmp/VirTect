...
task RunVirTect {
    ...
    command <<<
        ...
        python <<CODE
            ...
            file =open(~{prefix}+"/continuous_region.txt", "r")
            out_put =open(~{prefix}+"/Final_continous_region.txt", "w")
            ...
            final_output=open(~{prefix}+"/Final_continous_region.txt",'r')
            if (os.fstat(final_output.fileno()).st_size) >0:
                ...
                for line in fileinput.input([~{prefix}+'/Final_continous_region.txt'], inplace=True):
